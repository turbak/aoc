...
impl Grid {
    ...
    fn drop_sand_grain(&mut self) -> bool {
        let i_max = *self.grid_map.iter().map(|((i, _), _)| i).max().expect("should have found max_i val");
        let j_max = *self.grid_map.iter().map(|((_, j), _)| j).max().expect("should have found max_j val");
        let j_min = *self.grid_map.iter().map(|((_, j), _)| j).min().expect("should have found min_j val");
...
impl Display for Grid {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let i_max = *self.grid_map.iter().map(|((i, _), _)| i).max().expect("should have found max_i val");
        let j_max = *self.grid_map.iter().map(|((_, j), _)| j).max().expect("should have found max_j val");
        let i_min = *self.grid_map.iter().map(|((i, _), _)| i).min().expect("should have found min_i val");
        let j_min = *self.grid_map.iter().map(|((_, j), _)| j).min().expect("should have found min_j val");
